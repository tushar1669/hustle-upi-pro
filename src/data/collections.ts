...
// 🚀 NEW: minimal helpers for Projects (non-breaking)
export async function update_project(
  id: string,
  changes: Partial<{ client_id: string; name: string; is_billable: boolean }>
) {
  ...
  if (filters.when) {
    ...
    switch (filters.when) {
      ...
      case "next_7_days": {
        ...
        query = query.gte("scheduled_at", today.toISOString()).lt("scheduled_at", nextWeek.toISOString());
        break;
      }
      case "overdue":
...
export async function bulk_update_reminders(
  ...
  const { data, error } = await supabase.from("reminders").update(changes).in("id", ids).select("*");
  if (error) throw error;
  return data || [];
}
...
export async function message_log_recent() {
  const { data, error } = await supabase.from("message_log").select("*").order("sent_at", { ascending: false }).limit(20);
  if (error) throw error;
  return data || [];
}
...
// ============ Savings Goals (NEW minimal helpers) ============
...
export async function create_savings_goal(payload: {
  ...
  type?: string | null; // e.g., 'tax', 'general'
}) {
...
export async function update_savings_goal(
  ...
  const { data, error } = await supabase.from("savings_goals").update(changes).eq("id", id).select("*").single();
  if (error) throw error;
  return data;
}
